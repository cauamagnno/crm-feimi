...
const TeamConfigModal: React.FC<TeamConfigModalProps> = ({ isOpen, onClose, onUpdate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-card border border-border rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto max-h-[50vh] custom-scrollbar">
          {loading ? (
            ...
            <div className="space-y-3">
              ...
              {isCreating ? (
                ...
                <button
                  ...
                  className="w-full bg-muted/30 border border-dashed border-border rounded-lg p-4 text-muted-foreground hover:text-foreground hover:border-slate-600 transition-colors flex items-center justify-center gap-2"
                >
              ...
              {/* Teams List */}
              {teams.map((team) => (
                <div key={team.id} className="bg-muted/50 border border-border rounded-lg p-4">
                  {editingId === team.id ? (
                    <div className="space-y-3">
                      <input
                        type="text"
            ...
            <div className="space-y-3">
              ...
              {/* Functions List */}
              {functions.map((func) => (
                <div key={func.id} className="bg-muted/50 border border-border rounded-lg p-4">
                  {editingId === func.id ? (
                    <div className="space-y-3">
                      <input
                        type="text"
